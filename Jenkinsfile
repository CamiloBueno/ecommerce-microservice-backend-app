                    ...
                    steps {
                        script {
                            ...
                            for (svc in services) {
                                ...
                                def healthCheck = "powershell -Command \"\$i=0; while (\$i -lt 30) { try { \$resp = Invoke-WebRequest -Uri '${svc.health}' -UseBasicParsing -TimeoutSec 3; if (\$resp.StatusCode -eq 200) { Write-Host '${svc.name} is healthy'; break } } catch {} Start-Sleep -Seconds 2; \$i++ }\""
                                bat healthCheck
                            }
                        }
                    }
